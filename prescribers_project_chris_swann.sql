...
GROUP BY 
	p.specialty_description
HAVING 
	SUM(pr.total_claim_count) IS NULL;
...
SELECT
	...
	ROUND(SUM(CASE WHEN d.opioid_drug_flag = 'Y' THEN p.total_claim_count ELSE 0 END) /
	SUM(p.total_claim_count) * 100, 2) AS percent_opioid_claims
FROM
	prescriber pb
INNER JOIN
...
SELECT
	...
	SUM(total_claim_count)) * 100 AS percent_opioid
FROM
	prescriber
	LEFT JOIN 
		prescription USING (npi)
...
SELECT
	generic_name,
	(SUM(total_drug_cost) / SUM(total_day_supply))::MONEY AS total_cost
FROM 
	drug
	INNER JOIN
		prescription USING (drug_name)
...
CROSS JOIN
	drug
LEFT JOIN
	prescription USING (npi, drug_name)
WHERE 
	specialty_description = 'Pain Management'
	AND nppes_provider_city = 'NASHVILLE'
	AND opioid_drug_flag = 'Y';
